...
function Delete(){
	if(UserFunctions::hasComponentAccess("menus", "delete") == true){
		...
		if($id > 0){
			...
			if($data->canDelete() == true){
				if($result == 0){
					...
					echo Text::pageTitle("Delete Menu");
					?>
					...
					<form name="adminForm" id="adminForm" method="post" action="<?php echo Url::getAdminHttpBase(); ?>/index.php?option=menus&act=delete&id=<?php echo $id; ?>&result=1">
				...
				if($result == 1){
					...
					if($data->delete() == true){
						Messages::setMessage("Menu Deleted", Define::get("MessageLevelSuccess"));
					}
					
					Url::redirect(Url::getAdminHttpBase() . "/index.php?option=menus&act=manage", 0, false);
				}
			} else {
				Messages::setMessage("Cannot delete menu", Define::get("MessageLevelError"));
				Url::redirect(Url::getAdminHttpBase() . "/index.php?option=menus&act=manage", 0, false);
			}
		} else {
