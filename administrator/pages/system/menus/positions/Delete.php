...
function Delete(){
	if(UserFunctions::hasComponentAccess("menupositions", "delete") == true){
		...
		if($id > 0){
			...
			if($data->canDelete() == true){
				if($result == 0){
					...
					echo Text::pageTitle("Delete Menu Position");
					?>
					...
					<form name="adminForm" id="adminForm" method="post" action="<?php echo Url::getAdminHttpBase(); ?>/index.php?option=menupositions&act=delete&id=<?php echo $id; ?>&result=1">
				...
				if($result == 1){
					...
					if($data->delete() == true){
						Messages::setMessage("Menu Position Deleted", Define::get("MessageLevelSuccess"));
					}
					
					Url::redirect(Url::getAdminHttpBase() . "/index.php?option=menupositions&act=manage", 0, false);
				}
			} else {
				Messages::setMessage("Cannot delete menu position", Define::get("MessageLevelError"));
				Url::redirect(Url::getAdminHttpBase() . "/index.php?option=menupositions&act=manage", 0, false);
			}
		} else {
