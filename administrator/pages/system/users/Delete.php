...
function Delete(){
	if(UserFunctions::hasComponentAccess("users", "delete") == true){
		...
		if($id > 0){
			...
			if($data->canDelete() == true){
				if($result == 0){
					...
					echo Text::pageTitle("Delete User");
					?>
					...
					<form name="adminForm" id="adminForm" method="post" action="<?php echo Url::getAdminHttpBase(); ?>/index.php?option=users&act=delete&id=<?php echo $id; ?>&result=1">
				...
				if($result == 1){
					...
					if($data->delete() == true){
						Messages::setMessage("User Deleted", Define::get("MessageLevelSuccess"));
					}
					
					Url::redirect(Url::getAdminHttpBase() . "/index.php?option=users&act=manage", 0, false);
				}
			} else {
				Messages::setMessage("Cannot delete user", Define::get("MessageLevelError"));
				Url::redirect(Url::getAdminHttpBase() . "/index.php?option=users&act=manage", 0, false);
			}
		} else {
