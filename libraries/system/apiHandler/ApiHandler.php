 ...
 * TODO: Add list of urls that can call the handler
 */
class ApiHandler extends Library {
	protected $recordInfo = array('id' => 0);
	private $recordText = array('id' => "ID");
	private $recordGets = array('id' => "getId");
	
	public function ApiHandler($inputId = NULL){
		if(isset($inputId)){
	...
	private function buildData(){
		...
		if(self::getId() > 0){
			...
			$info = $db->fetchAssoc("SELECT * FROM apiHandler WHERE id='" . self::getId() . "'");
			
			$this->recordInfo['handlerId'] = $info->handlerId;
			$this->recordText['handlerId'] = "API Handler ID";
			...
			$this->recordInfo['restricted'] = $info->restricted;
			$this->recordText['restricted'] = "Restricted";
			$this->recordGets['restricted'] = "getRestricted";
		}
	}
	...
	/**
	 * Whether or not this function can be edited or deleted
	 * TODO: Rename to something more appropriate (systemFunction)
	 */
	public function getRestricted($text = 0){
		if($text == 1){
			return self::getYesNoText($this->recordInfo['restricted']);
		} else {
			return $this->recordInfo['restricted'];
		}
	}
	 ...
	 * Whether or not this function can be edited or deleted
	 */
	public function setRestricted($inputValue){
		if(isset($inputValue)){
			$this->recordInfo['restricted'] = $inputValue;
			return true;
		}
	...
	public function delete(){
		...
		if($this->recordInfo['id'] > 0 && self::canDelete() == true){
			$result = $db->delete("DELETE FROM apiHandler WHERE id=" . addslashes(self::getId()));
			
			if($result == true){
				// Log it
				Log::action("API Handler Deleted by " . UserFunctions::getLoggedIn()->getUsername());
				return true;
			}
		}
	...
	public function save(){
		...
		if($this->recordInfo['id'] > 0){
			$result = $db->update("UPDATE apiHandler SET " . 
			...
			if($result == true){
				...
				Log::action("API (" . self::getId() . ") edited: " . ($changes != "" ? $changes : "None"));
			}
		} else {
			$result = $db->insert("INSERT INTO apiHandler (handlerId, name, className, callerFunction, createTimestamp, expireTimestamp, guestAccess) VALUES (" . 
			...
			if($result == true){
				...
				Log::action("API Handler (" . self::getId() . ") added");
			}
		}
	...
	private function generateUniqueId(){
		...
		$count = $db->fetchObject("SELECT id FROM apiHandler WHERE id='" . $uniqueId . "'");
